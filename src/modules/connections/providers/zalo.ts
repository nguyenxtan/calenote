...
export async function setZaloWebhook(token: string, input: WebhookRegistration, requester: ProviderRequester = postSecretProviderJson): Promise<void> {
  ...
  if (!isRecord(payload.result) || !isRecord(payload.result.verification) || typeof payload.result.verification.ok !== "boolean") throw new ProviderOperationError("INVALID_RESPONSE");
  if (!payload.result.verification.ok) throw new ProviderOperationError("FAILED");
}
export async function sendZaloText(token: string, chatId: string, text: string, requester: ProviderRequester = postSecretProviderJson): Promise<SendReceipt> { if (!chatId || text.length < 1 || text.length > 2000) throw new ProviderVerificationError("INVALID_PROVIDER_RESPONSE"); const payload = await requester({ provider: "zalo", hostname: ZALO_BOT_API_HOSTNAME, path: path(token, "sendMessage"), operation: "sendMessage", body: { chat_id: chatId, text } }); if (!isRecord(payload) || typeof payload.ok !== "boolean") throw new ProviderOperationError("INVALID_RESPONSE"); if (!payload.ok) throw providerOperationFailureFromPayload("zalo", payload); const result = isRecord(payload.result) ? payload.result : null; if (!result || (typeof result.message_id !== "string" && typeof result.message_id !== "number")) throw new ProviderOperationError("INVALID_RESPONSE"); return { providerMessageId: String(result.message_id) }; }
