...
export async function setTelegramWebhook(token: string, input: WebhookRegistration, requester: ProviderRequester = postSecretProviderJson): Promise<void> {
  if (!isCanonicalBase64Url(input.secretToken, 1)) throw new ProviderVerificationError("INVALID_PROVIDER_RESPONSE");
  ...
  if (!isRecord(payload) || typeof payload.ok !== "boolean") throw new ProviderOperationError("INVALID_RESPONSE");
  if (!payload.ok) throw providerOperationFailureFromPayload("telegram", payload);
  if (payload.result !== true) throw new ProviderOperationError("INVALID_RESPONSE");
}
export async function sendTelegramText(token: string, chatId: string, text: string, requester: ProviderRequester = postSecretProviderJson): Promise<SendReceipt> { if (!chatId || text.length < 1 || text.length > 4096) throw new ProviderVerificationError("INVALID_PROVIDER_RESPONSE"); const payload = await requester({ provider: "telegram", hostname: TELEGRAM_BOT_API_HOSTNAME, path: path(token, "sendMessage"), operation: "sendMessage", body: { chat_id: chatId, text } }); if (!isRecord(payload) || typeof payload.ok !== "boolean") throw new ProviderOperationError("INVALID_RESPONSE"); if (!payload.ok) throw providerOperationFailureFromPayload("telegram", payload); const result = isRecord(payload.result) ? payload.result : null; if (!result || (typeof result.message_id !== "string" && typeof result.message_id !== "number")) throw new ProviderOperationError("INVALID_RESPONSE"); return { providerMessageId: String(result.message_id) }; }
