...
function path(token: string, operation: "getMe" | "setWebhook" | "sendMessage"): string { if (!isSafeProviderToken("telegram", token)) throw new ProviderVerificationError("INVALID_TOKEN_FORMAT"); return `/bot${token}/${operation}`; }
export async function setTelegramWebhook(token: string, input: WebhookRegistration, requester: ProviderRequester = postSecretProviderJson): Promise<void> { if (!/^[A-Za-z0-9_-]{1,256}$/u.test(input.secretToken)) throw new ProviderVerificationError("INVALID_PROVIDER_RESPONSE"); const payload = await requester({ provider: "telegram", hostname: TELEGRAM_BOT_API_HOSTNAME, path: path(token, "setWebhook"), operation: "setWebhook", body: { url: input.url, secret_token: input.secretToken, allowed_updates: ["message"] } }); if (!isRecord(payload) || payload.ok !== true) throw providerOperationFailureFromPayload("telegram", isRecord(payload) ? payload : {}); }
