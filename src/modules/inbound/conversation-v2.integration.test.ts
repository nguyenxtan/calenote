...
async function harness() {
  ...
  const process = (message: BoundChatMessage) => processBoundChatMessage(message, { store: commandStore,
    keyring: h.keyring, now: () => time, reply: async text => { replies.push(text); }, conversation: service });
  const send = async (text: string, model = create()) => {
    ...
    return { result: await process(message), message, reply: replies.at(-1) };
  };
