...
if (runIdIndex === -1 || !args[runIdIndex + 1] || !["legacy-pair", "gemini-pilot", "gemini-flash-pilot", "gemini-flash-full"].includes(profile)
  || args.some((arg, index) => !["--preflight", "--run-id", "--profile"].includes(arg) && index !== runIdIndex + 1 && index !== profileIndex + 1)) {
  throw new TypeError("Usage: run-semantic-v1-live.mjs [--preflight] [--profile legacy-pair|gemini-pilot|gemini-flash-pilot|gemini-flash-full] --run-id <safe-run-id>");
}
...
const flash = { candidateId: "gemini-2.5-flash", model: "google/gemini-2.5-flash", provider: "google-vertex/eu", reasoning: "DISABLED", promptPriceMicrounitsPerMillionTokens: 300_000, completionPriceMicrounitsPerMillionTokens: 2_500_000 };
const profileOptions = profile === "gemini-pilot"
  ...
  : profile === "gemini-flash-pilot"
    ? { candidates: [flash], caseIds: GEMINI_PILOT_CASE_IDS, maxHttpRequests: 40, maxCostMicrounits: 210_000 }
  : profile === "gemini-flash-full"
    ? { candidates: [flash], maxHttpRequests: 220, maxCostMicrounits: 1_210_000 }
  : { candidates: [
